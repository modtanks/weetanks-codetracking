...
public class MissionHundredController : MonoBehaviour
{
	...
	public List<GameObject> BlocksToMoveDownAfterThroneDestroyed = new List<GameObject>();
	...
	public void MoveArenaBlocksDown()
	{
		Debug.Log("moving blocks down/");
		if (!IsUp)
		{
			return;
		}
		Debug.Log("in floor!");
		foreach (GameObject item in BlocksToMoveDownAfterThroneDestroyed)
		{
			StartCoroutine(MoveBlockDown(item));
		}
		IsUp = false;
	}

	public void MoveArenaBlocksUp()
	{
		if (IsUp)
		{
			return;
		...
		foreach (GameObject item in BlocksToMoveDownAfterThroneDestroyed)
		{
			StartCoroutine(MoveBlockUp(item));
		}
		IsUp = true;
	}
}
